...
import BalanceSheetView, { displayAmount } from "../components/BalanceSheetView";
...
function BalanceSheet() {
  ...
  async function loadBalanceSheet() {
    ...
    try {
      const { data } = await api.get("/api/financial/balance-sheet", {
        params: {
          year: year || undefined,
      ...
      console.error(err);
      setError("Failed to load the balance sheet. Please try again.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-800">Balance Sheet</h1>
          <p className="text-slate-500 mt-2">
            Assets, Liabilities, and Owners Equity as of the selected
            reporting period.
          </p>
        </div>
