...
import StatementOfEquityView from "../components/StatementOfEquityView";
...
function downloadCsv(data) {
  ...
  link.href = url;
  link.setAttribute("download", "statement-of-equity.csv");
...
function StatementOfEquity() {
  ...
  useEffect(() => {
    loadStatementOfEquity();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [year, quarter, month, country]);

  async function loadStatementOfEquity() {
    setLoading(true);
    setError(null);

    try {
      const { data } = await api.get("/api/financial/statement-of-equity", {
        params: {
          year: year || undefined,
      ...
      console.error(err);
      setError(
        "Failed to load the statement of equity. Please try again."
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-2">
            Movements in Share Capital, Retained Earnings, and Dividends
      ...
      {loading ? (
        ...
        <StatementOfEquityView data={data} />
      )}
    </div>
  );
}
